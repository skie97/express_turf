...
test('getIntersection: getSector with single position and no boundary', () => {
    ...
    const firstSighting: sightingData = {
        sightingDateTime: addHours(currentTime, -1),
        avgSpeedInKmh: 1,
    ...
    const result = getIntersection([firstSighting], currentTime)
    console.log(result.geometry.coordinates);
    console.log(getIntersection([secondSighting], currentTime).geometry.coordinates);
})